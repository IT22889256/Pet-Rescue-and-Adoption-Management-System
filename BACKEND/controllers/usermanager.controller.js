...
const createEmployee = asyncHandler(async (req, res, next) => {
  console.log("manager", req.body);
  const { name, email, password, role, eid, roletype } = req.body; // Make sure to include 'role' if it's being sent from the front end
  try {
    // Validation
    if (!name || !email || !password) {
      res.status(400);
    ...
    if (employee) {
      const { _id, name, email, role, eid, roletype } = employee;
      res.status(201).json({
        _id,
        name,
        email,
