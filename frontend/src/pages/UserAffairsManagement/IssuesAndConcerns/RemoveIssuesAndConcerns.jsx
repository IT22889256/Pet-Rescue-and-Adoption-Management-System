...
export default function DeleteIC() {
  const navigate = useNavigate();
  const { id } = useParams();
  const handleDeleteIC = () => {
    axios
      .delete(
        `http://localhost:3000/userAffairsManager/issueandconcern/deleteIssueAndConcern/${id}`
      )
      .then(() => {
        alert("deleted");
      ...
      <div className="w-full max-w-md bg-white shadow-lg rounded-md p-6 relative">
        ...
        <div className="flex flex-col space-y-2">
          <button
            ...
            onClick={handleDeleteIC}
          >
            Yes, Delete it
          </button>
          <Link
            to={`/UserAffairsManager/handleIssuesConcerns`}
